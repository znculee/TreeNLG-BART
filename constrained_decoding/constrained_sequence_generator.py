...
class ConstrainedSequenceGenerator(SequenceGenerator):
    def __init__(self, src_dict, tgt_dict, encoder_json, vocab_bpe, order_constr, **kwargs):
        super().__init__(tgt_dict, **kwargs)
        self.src_dict = src_dict
        self.tgt_dict = tgt_dict
        self.order_constr = order_constr
        self.bpe_encoder = get_encoder(encoder_json, vocab_bpe)
        self.nt_map = self._get_nt(tgt_dict)
        self.constraint_penalty = None

    def _bpe_decode(self, bpe_id, lstrip=True):
        """
        map gpt2 bpe id to token
        Note that bpe id may be <pad> <s> </s> ...
        """
        try:
            if lstrip:
                return self.bpe_encoder.decode([int(bpe_id)]).lstrip()
            else:
                return self.bpe_encoder.decode([int(bpe_id)])
        except ValueError:
            return bpe_id

    def _bpe_decode_seq(self, s):
        """
        map bpe id string to bpe token string
        """
        return ' '.join([self._bpe_decode(x) for x in s.split()])
    ...
    def _get_nt(self, vocab):
        ...
        """
        nt_map = []
        for i, bpe_id in enumerate(vocab):
            if i >= len(vocab):
                break
            tok = self._bpe_decode(bpe_id, lstrip=False)
            if tok.startswith(' [__') or tok == ' ]':
                nt_map.append(i)
        nt_map.append(self.tgt_dict.eos())
        return nt_map
    ...
    def _build_constraints(self, src_tokens, beam_size):
        ...
        srcs = [s.replace(self.tgt_dict[self.tgt_dict.pad()], "") for s in srcs]
        srcs = [s.replace(self.tgt_dict[self.tgt_dict.eos()], "") for s in srcs]
        constraints = [TreeConstraints(self._bpe_decode_seq(s), self.order_constr) for s in srcs]
        bbeam_constraints = []
    ...
    def _apply_constraints(self, lprobs, constraints, _idx):
        """
        ...
        for con in constraints:
            ...
            constraint_penalty.append([
                0.0
                if self._bpe_decode(self.tgt_dict[nt]).strip('[') in nominated_nt
                else -math.inf
    ...
    def _update_constraints(self, constraints, next_tokens, idx):
        ...
        for constraint, tok_id in zip(constraints, next_tokens):
            tok = self._bpe_decode(self.tgt_dict[tok_id]) if tok_id in self.nt_map else '_'
            assert constraint.next_token(tok, idx)
    ...
    def _generate(
        ...
        **kwargs
        ...
        def is_finished(sent, step, _unfin_idx):
            """
