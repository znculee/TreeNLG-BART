...
pct=pct-$1
dest=$pct/shuf.plbl.rrk-rev.itr$2

if [[ "$2" == "1" ]]; then
  ckpt=self_training/checkpoints/$data/$pct/shuf.lbl.$model/checkpoint_best.pt
  REVMDLDIR=self_training/checkpoints/$data/$pct/shuf.lbl.rrk-rev.rev.$model
else
  itrprev=$(bc <<< "$2-1")
  ckpt=self_training/checkpoints/$data/$pct/shuf.lbl.rrk-rev.itr$itrprev.$model/checkpoint_best.pt
  REVMDLDIR=self_training/checkpoints/$data/$pct/shuf.lbl.rrk-rev.rev.itr$itrprev.$model
fi
