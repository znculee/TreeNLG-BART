...
if [[ "$stg" == "lbl" ]]; then
  ...
  if [[ "$2" == "0" ]]; then
    echo "Please use train.weather.bart.sh to train the base model."
    exit 1
  else
    savedir=self_training/checkpoints/$data/$datadir.rrk-rev.$itr.$model
    restore=self_training/checkpoints/$data/$pct/shuf.plbl.rrk-rev.$itr.$model/checkpoint_best.pt
  fi
else
  datadir=$pct/shuf.plbl.rrk-rev.$itr
